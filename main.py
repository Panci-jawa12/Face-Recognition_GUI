...
class PresensiPage(tk.Frame):
    ...
    def predict(self, frame):
        ...
        if len(face_detec) == 1:
            now = datetime.now()
            try:
                predictions = predict(frame, face_detec, model_path=NAMA_MODEL)
            except Exception as e:
                print(e)
            if predictions and predictions != 'Unknown':
                user, data = check_user(predictions, now)
                if user and not data:
                    self.show_data(data=user)
                else:
                    content = {'suhu': self.suhu, 'jam': now, 'nama': predictions, 'masuk': data[0], 'pulang': data[1]}
                    try:
                        response = requests.post(f'{API_URL}/presensi/', data=content).json()
                        add_user(response)
                        self.show_data(data=response)
                    except Exception as e:
...
class TrainPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Button(self, text="START CAPTURE", command=self.start_capture, font=BUTTON_FONT, relief="solid", width=33, height=2).place(x=350, y=635)
        tk.Button(self, text="TRAIN MODEL", command=self.train, font=BUTTON_FONT, relief="solid", width=33, height=2).place(x=691, y=635)
        tk.Button(self, text="< Kembali", relief="flat", cursor="hand2", command=self.back, font=BUTTON_FONT).place(x=25, y=25)
    ...
    def captured(self, frame, resize_frame):
        if self.capture:
            ...
            face_detec = face_recognition.face_locations(resize_frame)
            if len(face_detec) == 1:
                filename = f"files/user/{self.nama}_{self.count}.jpg"
                try:
                    cv2.imwrite(filename, cv2.cvtColor(resize_frame, cv2.COLOR_RGB2BGR))
                except Exception as e:
                    print(e)
                self.files[f"img{self.count}"] = filename
                self.count+=1
                cv2.putText(resize_frame, 'CAPTURED', (70,70), cv2.FONT_HERSHEY_SIMPLEX , 1, (255, 0, 0), 2, cv2.LINE_AA)
            else:
